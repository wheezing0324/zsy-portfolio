...
export const prd = {
  feedback: [
    { role: "运营负责人", text: "客户续费前想看到各门店使用活跃度和异常账号，最好能按行业和客户等级筛选。" },
    { role: "客户成功", text: "每次导出数据都要找研发，内部同学需要一个固定口径的报表入口。" },
    { role: "测试同学", text: "筛选条件和导出字段需要提前确定，否则验收时很容易反复。" }
  ],
...
export const baseFeedbackCount = 9;

export const acceptanceState = {
  checks: [
  ...
  feedback: [
    { title: "运营：希望导出字段可保存为常用模板", note: "产品判断：高频但非首版阻塞，进入 P1 迭代。", status: "P1 / 下一版" },
    { title: "测试：风险原因枚举与接口返回不一致", note: "产品判断：影响验收口径，需本轮修复并同步字段说明。", status: "P0 / 本轮修复" },
    { title: "客户成功：需要一页使用说明给一线同学", note: "产品判断：补充版本发布文档和培训话术。", status: "文档补充" }
  ]
};

export const getAcceptanceSummary = () => {
...
export const getHomepagePayload = () => ({
